...
app.get('/api/fetch-arxiv', async (req, res) => {
  ...
  try {
    ...
    const response = await fetch(url, {
      headers: { 'User-Agent': 'Litmap/1.0 (research tool)' },
    })
...
app.post('/api/ask', async (req, res) => {
  ...
  if (!node || !question) return res.status(400).json({ error: 'Missing node or question' })
...
app.listen(PORT, () => {
  console.log(`Litmap API server running on http://localhost:${PORT}`)
})
