...
export function PaperInput({ onPaperLoaded }: Props) {
  ...
  const handleUrl = async () => {
    ...
    if (!arxivId) {
      setError('Could not find an arXiv ID in that URL. Try: arxiv.org/abs/2301.12345')
      return
    }
    setError(null)
    setLoading(true)
    ...
    try {
      ...
      if (!fetchRes.ok) throw new Error(`arXiv fetch failed: ${fetchRes.statusText}`)
      const { text } = await fetchRes.json()
      setStatus('Extracting knowledge graph with Claude…')
